function [P,Out] = ncp_imu(X,R,varargin)
%NCP_IMU: Nonnegative CANDECOMP/PARAFAC tensor decomposition using
% multiplicative update method in block coordinate descent framework.
%
...
if iscell(init)
    ...
    if strcmp(init,'random')
        Uinit = cell(N,1);
        for n = dimorder(1:end)
            Uinit{n} = max(0,randn(size(X,n),R)) + 0.1; % randomly generate each factor
        end
...
start_time = tic;
if printitn>=0
    fprintf('\nNonnegative CANDECOMP/PARAFAC using MU:\n');
end
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        ...
        [Unew, iterU] = solver_mu(MTTKRP',BtB',U{n}','tol',tolU(n),'beta',regparams(n,2),'maxiters',inner_iter);
        if iterU==1
            tolU(n) = 0.1*tolU(n);
        end
