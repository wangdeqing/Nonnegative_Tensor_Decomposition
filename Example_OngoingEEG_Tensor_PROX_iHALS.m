% Ongoing EEG tensor decomposition using PROX-iHALS algorithm.
% Author: Deqing Wang
...
% Tensor Decomposition Parameters
% Parameter format: [alpha_n, beta_n]
% alpha_n: proximal regularization parameter,
%          the default value for ANQP is 1e-4.
% beta_n: sparse regularization parameter
%
% Example values for the ongoing EEG: SparseParam = 0, 1e5, 5e5, 10e5, 15e5, 20e5
SparseParam = 0;
RegParams = repmat([1e-4 SparseParam],N,1);

%% Start of the NCP tensor decomposition
rng('shuffle');
[A,Out] = ncp_prox_ihals(TensorTrue,R,'maxiters',99999,'tol',1e-8,...
