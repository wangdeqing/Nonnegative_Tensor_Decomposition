function [P,Out] = ncp_prox_ihals(X,R,varargin)
%NCP_PROX_IHALS: Nonnegative CANDECOMP/PARAFAC tensor decomposition using
% proximal algorithm hierarchical alternating least square in inexact block
% coordinate descent coordinate descent framework.
%
...
params.addParameter('inner_iter', 5, @(x) (isscalar(x) & x > 0));
...
start_time = tic;
if printitn>=0
    fprintf('\nNonnegative CANDECOMP/PARAFAC using PROX-iHALS:\n');
end
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        ...
        [Unew,iterU] = solver_hals(MTTKRP_r',BtB',U{n}',iter,'tol',tolU(n),'alpha',regparams(n,1),'maxiters',inner_iter);
        if iterU==1
            tolU(n) = 0.1*tolU(n);
        end
